...
local function getmetafile(url)
    for i = 1, #metafileFormats do
        ...
        if fileExists(f) then return f end
    end
...
function iffy.newAtlas(name, url, metafile, sw, sh)
    ...
    if url then
        if type(url) ~= "string" then
            ...
            if not metafile then metafile = getmetafile(url) end
        end
    else
    ...
    if getExtension(metafile) == "xml" then
        ...
        for line in love.filesystem.lines(metafile) do
            if i > 1 and line:match("%a") and not line:match("<!") and line ~=
                "</TextureAtlas>" then
                _, sname = string.match(line, "name=([\"'])(.-)%1")

                assert(not t[sname], "Iffy Error!! Duplicate Sprite Names (" .. sname ..
                    ") for " .. name)
    ...
    for i = 1, tiles_h do
        for j = 1, tiles_w do
            ...
            t[current] = love.graphics.newQuad(
                ...
                (i - 1) * tw + (current == 1 and 0 or my), tw, th, sw, sh)
        end
    end
    iffy.spritesheets[name] = t
...
function iffy.newTilemap(name, url)
    local t = {}
    if not url then
        assert(name, "Iffy Error!! You must pass atleast one argument in 'newTilemap'!!")
        url = name
        name = removeExtension(url)
    end
    if type(url) == "string" then
        assert(fileExists(url),
            "Iffy Error! The provided file '" .. url .. "' doesn't exist")
    ...
    for i = 1, #iffy.tilemaps[map_name][1] do
        for j = 1, #iffy.tilemaps[map_name] do
            ...
            if iffy.tilemaps[map_name][j][i] + offset > 0 then
                iffy.draw(tileset_name, iffy.tilemaps[map_name][j][i] + offset,
...
function iffy.exportXML(iname, path, filename)
    ...
    for i = 1, #iffy.spritedata[iname] do
        ...
        file:write(string.format(
            "\t<SubTexture name=\"%s\" x=\"%s\" y=\"%s\" width=\"%s\" height=\"%s\"/>\n",
